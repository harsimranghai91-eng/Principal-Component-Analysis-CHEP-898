...
Interpretation: The interpretation of some of the components are as follows based on the loading value of each variables on the principal component. We used the criteria of loading greater than 0.2 and less than -0.2 to interpret the factors. The first factors based on the loading can be interpreted as physical activity, the second variable being anthropometric, the third variable can be interpreted as sitting and walk time and fourth component is fruits and vegetable quantity. All other components are hard to interpret.

## Variance explained by principal components and selecting the components
...
The first component explains 20.53 percent of the variance, the second component explain 16.7 and the third component explain 10.7 percent of the variance. Using on the kaiser criterion ( based on the scree plot) or elbow method, I believe that getting three components out for the regression analysis would be better. However, a more robust approach will be to do parallel analysis and selecting the component based on that analysis (this is not done in this assignment).
...
The three component (extracted using the elbow method) explain approximately 49 % of the variance in the data set, which is not good, but for the sake of the practice, I will continue with the analysis.
...
The initial regression analysis will be done without choosing the component. After that, regression would be conducted with only three components (tuned model) and then the model will be compared based on their performance on the testing dataset. Models will also be compared based on residuals analysis.
...
Interpretation: None of categorical variables were statistically significant, however, many principal components were found to be statistically associated with dependent variables with some having positive association and other having negative association. To truly define the relationship, it is imperative to interpret the factors first to make sense of the relationships observed in this regression analysis. however, I was only able to interpret the first few components. The first component which was interpreted as physical activity tell us that given a unit increase in this component, the dependent variable will decrease by 0.619 units. The second component which was interpreted as anthropometric component tell us that given a unit increase in anthropometric component, the dependent variable will decrease by 2.375 units. The third component which was interpreted as sitting and walk time tell us that given a unit increase in this component, the dependent variable will decrease by 0.409 units. The fourth component which is fruits and vegetable quantity is positively associated with dependent variable, which tell us that given a unit increase in fruit and vegetable quantity, the dependent variable will increase by 0.529 units.
...
Interpretation: None of the categorical variables was found to be associated with dependent variable (all P>0.05). However, all the principal components were statistically significant and showed negative association with the dependent variable. The first component which was interpreted as physical activity tell us that given a unit increase in this component, the dependent variable will decrease by 0.641 units. The second component which was interpreted as anthropometric component tell us that given a unit increase in anthropometric component, the dependent variable will decrease by 2.431 units. The third component which was interpreted as sitting and walk time tell us that given a unit increase in this component, the dependent variable will decrease by 0.403 units.
...
```{r}
## linear regression model using all components
bmi_fit |> 
  extract_fit_engine() |> 
  check_model()
```

Based on the residual plots, I believe that the assumption of linearity, homoscdesticity and normality of residuals are violated, which put questions on the inference based on this model.

```{r}
## linear regression model using only three components
bmi_fit2 |> 
  extract_fit_engine() |> 
  check_model()
```
...
Based on the residual analysis, both model seems to violate the assumptions underlying the linear model and therefore we will continue with RMSE analysis to select the best model

```{r}
# Linear model with all components
rmse_test<- yardstick::rmse(data=bmi_predicted, truth = PM_BMI_SR,
         estimate = .pred)
rmse_test

# Linear model with only three components
rmse_test2<- yardstick::rmse(data=bmi_predicted2, truth = PM_BMI_SR,
         estimate = .pred)
rmse_test2
```

Best Model: The RMSE for the linear PCA model with all components is less than the RMSE of the linear model with only three components.Therefore, I believe that the regression model with all the components is a better model.

# Limitations:
1. For the sake of assignments, I randomly choosed anthropometric and diet variables which may not be correlated as evident from the correlation analysis. In reality, the variables put in the PCA have issues of multicollinearity or they are related to each other.
2. Parallel analysis is a more robust method to choose components for PCA, however I didn't use that approach.
3. Regression models were compared solely based on their performance on testing dataset. In reality, a more detailed evaluation of the linear model is needed, along with the assumption testing to see which model is the best model. This is not done in this assignment.
4. The components due to lack of relationship between some of the variables are not interpretable.
5. we didn't use the rotation, it is possible that principal components will be more interpretable, if we use rotation techniques.
